...
function magnetism_posted_on() {
	...
	$byline = sprintf(
		esc_html_x( 'by %s', 'post author', 'magnetism' ),
...
function magnetism_entry_footer() {
	...
	if ( 'post' === get_post_type() ) {
		...
		/* translators: used between list items, there is a space after the comma */
		$tags_list = get_the_tag_list( '', esc_html__( ', ', 'magnetism' ) );
		if ( $tags_list ) {
	...
	if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
		...
		comments_popup_link( sprintf( wp_kses( __( 'Leave a Comment<span class="screen-reader-text"> on %s</span>', 'magnetism' ), array( 'span' => array( 'class' => array() ) ) ), get_the_title() ) );
		echo '</span>';
	}
